...
\subsubsection{Enabling GPU Usage}

In order to run the solver (and other parts) on the (NVIDIA) GPU you
need to add the following section to your input file. The parameters
may be adjusted for optimal performance.
...
Note that usually the GPU device is chosen automatically on hosts with
more than one GPU attached. If really needed you can specify the
device with {\ttfamily DeviceNum=2} to run on GPU device 2 (counting
starts with 0).
